...
# Override these via env if your local layout differs.
import os as _os
REPO_ROOT = Path(__file__).resolve().parents[1]
BULK_IN = Path(_os.environ.get("BULK_IN", "/tmp/bulk_added_genomes.jsonl"))
BULK_OUT = Path(_os.environ.get("BULK_OUT", "/tmp/bulk_added_genomes.enriched.jsonl"))
BASELINE = Path(_os.environ.get("BASELINE", REPO_ROOT / "web" / "hosts.json"))
PICKLE_DIR = Path(_os.environ.get("PICKLE_DIR", REPO_ROOT / "data" / "genomes"))

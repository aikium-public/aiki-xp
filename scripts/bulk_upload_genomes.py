...
Inputs:
  --src DIR     Local directory of `.pkl` genome pickles. Defaults to
                ./data/genomes relative to the repo root; override via
                --src or the GENOME_PICKLE_DIR env var.
...
Steps:
  1) Walk the source directory; for each .pkl, unpickle the SeqRecord and
     apply the same pseudogene-aware CDS filter as genome_lookup.py.
...
DEFAULT_SRC = Path(_os.environ.get("GENOME_PICKLE_DIR", _REPO_ROOT / "data" / "genomes"))
...
def count_valid_cds(record) -> tuple[int, int]:
    """Same pseudogene-aware filter as aikixp/genome_lookup.py."""
    n_valid = 0
    n_pseudo = 0
...
def modal_volume_put_dir(src_dir: Path, dst: str = "/") -> bool:
    ...
    baseline 1,831 pickles on the volume are never overwritten."""
...
def main() -> int:
    ...
    # pickles on the volume (baseline 1,831) are untouched.
    import tempfile

...
# Canonical sources (XP5Ensemble inference path; see aikixp.inference):
...
CAMBRAY_CLASSICAL = [
    ...
    # Both values are XP5Ensemble 5-fold transfers from 492K.
...
tier_spec = [
    ...
    #     configs/deployment_tiers.yaml).
    #
    # Tier B+ = R2 (Tier B body + ProtT5-XL + Evo-2 7B init_70nt, 8 mod)
    # at ρ_nc = 0.555 ± 0.014. An earlier init-window-only ablation at 0.538
    # with a "parallel branch" framing misrepresented the deployment
    # surface — a real user with upstream DNA also has the CDS that
    # follows the ATG.
...
# Panel e (a bespoke-recipe schematic) was dropped: its Cambray example
# at ρ=0.434 was not spectacular enough to warrant its own sub-panel.
# Giving panel d the full row width resolves the crowded y-axis and
# annotation overlap.
# ──────────────────────────────────────────────────────────────────
...
ax_d.text(1.0, -0.24,
          'All values: 5-fold XP5 ensemble, trained on 492K (Phase 5 & R1/R2)',
          transform=ax_d.transAxes, fontsize=5.2, ha='right', va='top',
          color='#777', style='italic')


# Panel e (bespoke recipe schematic) removed: it was redundant with the
# Cambray example already in panel d, and the example's ρ=0.434 was not
# spectacular enough to warrant its own sub-panel. Panel d now spans
# gs[2, :] (full row width).

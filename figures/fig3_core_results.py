#!/usr/bin/env python3
"""
Generate Aiki-XP publication figures.

Produces 6 main figures (multi-panel composites) + Extended Data figures.
All figures follow Nature-style figure guidelines:
...
Usage:
    python figures/fig3_core_results.py
"""
...
from _visual_data import INTERACTIVE_DIR, build_visual_data
...
# NM5 champion LOO (5-modality recipe, hard_hybrid campaign).
...
# 7B PCA-4096 champion F10 values, ESM-C unchanged (same checkpoint).
...
# NM5 champion missing-modality (hard_hybrid campaign Phase E).
...
def fig2_dataset_overview():
    ...
    # Strip shows only the conserved ribosomal component fraction;
    # the "Proteomics measured" row was removed because PaXDb was wrongly
    # treated as the only proteomics source (Abele is also proteomics).
...
def _parse_args():
    ...
    parser.add_argument(
        "--prepress",
        action="store_true",
        help="Also export high-resolution publication masters to figures/output/prepress.",
    )
    parser.add_argument(
        "--png-dpi",
        type=int,
...
def main():
    ...
    set_nbt_style()
    _get_visual_data()
    logger.info("Generating publication figures → %s", FIGURE_DIR)
    if EXPORT_CONFIG["write_prepress"]:

...
Invariants:
  - Pseudogene filtering. Any Bacformer cache built from the returned
    pickle MUST enumerate CDS in the filtered space:
  ...
  - Never overwrite the 1,831 baseline pickles. The caller
    (the /request_genome endpoint) checks file existence BEFORE calling
    and returns `already_cached` without touching disk.
  - No silent fallback: every failure raises GenomeDownloadError with a
    clear message.
"""
...
def _count_cds(record) -> tuple[int, int]:
    ...
    (pseudogene filtering invariant — see module docstring).
    """
    n_valid = 0
    n_pseudo = 0
...
def download_and_cache_genome(
    ...
    if pkl_path.exists() and not overwrite:
        raise GenomeDownloadError(
            ...
            "(baseline genomes are load-bearing for the lookup endpoints)."
        )

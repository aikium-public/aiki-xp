...
def compute_mega_component(
    ...
    # Bypass: set PROTEX_ALLOW_MEGA_FRACTION_SKEW=1 when training on an
    # intentional subset (e.g. a mega-only data-adaptation experiment).
...
def main():
    ...
    parser.add_argument(
        ...
        help='DEPRECATED — use --split-file explicitly. This flag points to a legacy '
             'split path that produced inconsistent Evo-2 7B profiling results in earlier runs.',
    )
    parser.add_argument(
    ...
    if args.use_production_operon_split:
        warnings.warn(
            ...
            "removed in a future release. This convenience flag was implicated in\n"
            "earlier Evo-2 7B profiling regressions caused by a wrong-split-file path.\n"
            "\n"
    ...
    if args.split_file is not None:
        ...
        # The balanced split is retained here as an explicit blocked SHA so
        # that an accidental --split-file pointing at it raises immediately.
        _BLOCKED_SPLIT_SHAS = {
            "2b6704a6afa4": "gene_cluster_operon_split_v2_balanced.tsv (DEPRECATED — "
                            "has 22 protein-leaked seqs and lacks compound_operon_id. "
                            "Use hard_hybrid_production_split_v2.tsv instead.)",
        }
    ...
    logger.info(
        ...
        "======================================================================\n"
        "----------------------------------------------------------------------\n"
        "  1. Back up the predictions parquet to your archive of choice.\n"
        "  2. If this beats the current champion, update\n"
        "     aikixp/champion_registry.py to point at the new checkpoint.\n"
        "======================================================================",
        output_file.parent,
    )
